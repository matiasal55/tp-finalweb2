...
//este metodo
    ...
    {
        $sql="SELECT `Usuarios`.`dni`,`Usuarios`.`nombre`,`Usuarios`.`apellido`,`Usuarios`.`tipo_licencia`, `Area`.`descripcion` FROM Usuarios,Area WHERE `Usuarios`.`cod_area`=`Area`.`codigo`";
        return $this->database->query($sql);
